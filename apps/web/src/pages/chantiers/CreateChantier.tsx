// src/pages/chantiers/CreateChantier.tsx
...
export default function CreateChantier() {
  ...
  const [form, setForm] = useState({
    ...
    commune: "",
    lieuDit: "",
    qualiteIds: [] as string[],   // ⚠️ on n’envoie plus essenceIds
    bucheronIds: [] as string[],
  });
  ...
  useEffect(() => {
    (async () => {
      ...
      setBucherons(u.map(x => ({ id: x.id, firstName: x.firstName, lastName: x.lastName })));
    })();
  }, []);

  // petit helper : nombre de qualités cochées par essence (affichage)
  const checkedByEssence = useMemo(() => {
    const map = new Map<string, number>();
    for (const ess of essences) {
      const ids = new Set(ess.qualites.map(q => q.id));
      const count = form.qualiteIds.reduce((acc, qid) => acc + (ids.has(qid) ? 1 : 0), 0);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (form.qualiteIds.length === 0) throw new Error("Choisis au moins une qualité");
      if (form.bucheronIds.length === 0) throw new Error("Choisis au moins un bûcheron");

      // ⚠️ On n’envoie PAS essenceIds : le back les déduit à partir des qualités
      const payload = {
      ...
      await api("/chantiers", { method: "POST", body: JSON.stringify(payload) });
      setOk("Chantier créé 🎉");
      setForm({
        referenceLot: "",
        convention: "",
      ...
      <form onSubmit={submit} className="grid gap-4 max-w-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Référence du lot">
            <input className="w-full border rounded px-3 py-2" value={form.referenceLot}
              onChange={e => setForm({ ...form, referenceLot: e.target.value })} required />
          </Field>
          <Field label="Convention">
            <input className="w-full border rounded px-3 py-2" value={form.convention}
              onChange={e => setForm({ ...form, convention: e.target.value })} required />
          </Field>
          <Field label="Propriétaire">
            <input className="w-full border rounded px-3 py-2" value={form.proprietaire}
              onChange={e => setForm({ ...form, proprietaire: e.target.value })} required />
          </Field>
          <Field label="Commune">
            <input className="w-full border rounded px-3 py-2" value={form.commune}
              onChange={e => setForm({ ...form, commune: e.target.value })} required />
          </Field>
          <Field label="Lieu-dit">
            <input className="w-full border rounded px-3 py-2" value={form.lieuDit}
              onChange={e => setForm({ ...form, lieuDit: e.target.value })} required />
          </Field>
        </div>
