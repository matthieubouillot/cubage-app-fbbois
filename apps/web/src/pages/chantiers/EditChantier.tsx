...
export default function EditChantier() {
  ...
  const [formData, setFormData] = useState<UpdateChantierDTO>({
    numeroCoupe: '',
    clientId: '',
    propertyId: '',
    qualityGroupIds: [],
  ...
  // États pour les protections (enquêtes sur les saisies)
  const [hasSaisies, setHasSaisies] = useState(false);
  const [qualityGroupsWithSaisies, setQualityGroupsWithSaisies] = useState<Set<string>>(new Set());
  const [bucheronsWithSaisies, setBucheronsWithSaisies] = useState<Set<string>>(new Set());
  const [debardeursWithSaisies, setDebardeursWithSaisies] = useState<Set<string>>(new Set());

  useEffect(() => {
    const loadData = async () => {
      try {
        setError(null);
        const [qualityGroupsData, clientsData, usersData, chantierData] = await Promise.all([
          getQualityGroups(),
          listClients(),
          listUsers(),
          getChantierById(id!)
        ...
        if (chantierData) {
          const c = chantierData as any;
          setFormData({
            numeroCoupe: c.numeroCoupe || '',
            clientId: c.client?.id || '',
            propertyId: c.property?.id || '',
            qualityGroupIds: c.qualityGroups?.map((qg: any) => qg.id) || [],
            bucheronIds: c.bucherons?.map((b: any) => b.id) || [],
            debardeurIds: c.debardeurAssignments?.map((d: any) => d.id).filter((id: string) => id) || [],
            lotConventions: []
          });
          
          // Définir le client sélectionné
          if (c.client) {
            setClientSearch(`${c.client.firstName} ${c.client.lastName}`);
          }
          
          // Charger les groupes de qualité sélectionnés
          const selectedQGs = qualityGroupsData.filter(qg => 
            c.qualityGroups?.some((cqg: any) => cqg.id === qg.id)
          );
          setSelectedQualityGroups(selectedQGs);
          
          // Charger les lots/conventions existants
          ...
          // Vérifier si le chantier a des saisies
          const qgsWithSaisies = new Set<string>();
          const bucheronsWithSaisiesSet = new Set<string>();
          const debardeursWithSaisiesSet = new Set<string>();
          let hasAnySaisies = false;

          if (c.qualityGroups && c.qualityGroups.length > 0) {
            for (const qg of c.qualityGroups) {
              try {
                const saisies = await listSaisies(c.id, qg.id);
                if (saisies && saisies.length > 0) {
                  hasAnySaisies = true;
                  qgsWithSaisies.add(qg.id);
                  
                  // Vérifier les bûcherons et débardeurs associés aux saisies
                  for (const saisie of saisies as any[]) {
                    const user = saisie.user;
                    if (user && user.roles && Array.isArray(user.roles) && user.id) {
                      if (user.roles.includes('BUCHERON')) {
                        bucheronsWithSaisiesSet.add(user.id);
                      }
                      if (user.roles.includes('DEBARDEUR')) {
                        debardeursWithSaisiesSet.add(user.id);
                      }
                    }
                  }
                }
              } catch (e) {
                console.error(`Erreur lors de la vérification des saisies pour le groupe ${qg.id}:`, e);
              }
            }
          }

          setHasSaisies(hasAnySaisies);
          setQualityGroupsWithSaisies(qgsWithSaisies);
          setBucheronsWithSaisies(bucheronsWithSaisiesSet);
          setDebardeursWithSaisies(debardeursWithSaisiesSet);
        }
  ...
  const handleQualityGroupToggle = (qualityGroup: QualityGroup) => {
    ...
    if (isSelected) {
      // Protection : ne pas permettre de désélectionner si le groupe a des saisies
      if (qualityGroupsWithSaisies.has(qualityGroup.id)) {
        setError('Impossible de supprimer ce groupe de qualité car il contient des saisies');
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateChantier(id!, submitData as any);
      navigate('/chantiers');
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <header className="text-center space-y-2">
          ...
          <div className="text-sm text-gray-500">
            Modifier les informations du chantier
          </div>
        </header>
            ...
            {bucherons.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {bucherons.map((bucheron) => {
                  ...
                  const hasData = bucheronsWithSaisies.has(bucheron.id);
                  return (
                    <label
                      key={bucheron.id}
                      ...
                      } ${hasSaisies && hasData && checked ? 'opacity-60' : ''}`}
                    >
                      <input
                        type="checkbox"
                        className="accent-black h-4 w-4"
                        checked={checked}
                        disabled={hasSaisies && hasData && checked}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setFormData(prev => ({
                              ...prev,
                            ...
                            // Protection : ne pas permettre de désélectionner si le bûcheron a des saisies
                            if (bucheronsWithSaisies.has(bucheron.id)) {
                              setError('Impossible de supprimer ce bûcheron car il a effectué des saisies');
                              return;
                            }
                            setFormData(prev => ({
                              ...prev,
            ...
            {debardeurs.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {debardeurs.map((debardeur) => {
                  ...
                  const hasData = debardeursWithSaisies.has(debardeur.id);
                  return (
                    <label
                      key={debardeur.id}
                      className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer hover:bg-gray-50 ${
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            // Protection : ne pas permettre de désélectionner si le débardeur a des saisies
                            if (debardeursWithSaisies.has(debardeur.id)) {
                              setError('Impossible de supprimer ce débardeur car il est associé à des saisies');
                              return;
                            }
                            setFormData(prev => ({
                              ...prev,
